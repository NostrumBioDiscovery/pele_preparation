...
def retrieve_receptor(complexfile, residue, ligand="ligand.pdb", receptor="receptor.pdb"):
    """
    This function returns receptor of the complex of interest.
    :param complex: system format pdb
    :output: receptor text
    """
    with open(complexfile, 'r') as pdb_file:
        lines = [line for line in pdb_file if line.startswith("ATOM") or line.startswith("HETATM")]
        receptor_text = [line for line in lines if line.startswith("ATOM") or (line.startswith("HETATM") and line[17:20].strip() != residue)]
    with open(complexfile, 'r') as pdb_file:
...
def parse_args():
    ...
    parser.add_argument("complex", type=str, help="Complex pdb")
    parser.add_argument("resname", type=str, help="Ligand resname")
    parser.add_argument("--poses", type=int, help="Number of ligand poses", default=40)
    args = parser.parse_args()
    return os.path.abspath(args.complex), args.resname, args.poses

if __name__ == "__main__":
   input, resname, poses  = parse_args()
   receptor, ligand = retrieve_receptor(input, resname)
   receptor_prepared, _, _, _ = PPP.main(receptor)
   output = randomize_starting_position(ligand, "ligand_input.pdb", resname,
           receptor_prepared, None, None, None, poses)
